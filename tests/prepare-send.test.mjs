...
test('prepare-send fails closed when a required linked root disappears, audits the reason, and succeeds after restore', t => {
  ...
  const initial = await prepareManagedSend(db, { workspaceId: 'ws-harness', provider: 'chatgpt', userPrompt: 'Use source.md', capture: capture('chatgpt', 'root-up', []) });
  assert.equal(initial.ok, true);

  fs.renameSync(linked, unavailable);
  const blocked = await prepareManagedSend(db, { workspaceId: 'ws-harness', provider: 'chatgpt', userPrompt: 'Use source.md again', capture: capture('chatgpt', 'root-up', []) });
  ...
  const restored = await prepareManagedSend(db, { workspaceId: 'ws-harness', provider: 'chatgpt', userPrompt: 'Retry source.md', capture: capture('chatgpt', 'root-up', []) });
...
test('an unavailable optional root cannot leak its last indexed version into a CURRENT send', t => {
  ...
  const initial = await prepareManagedSend(db, { workspaceId: 'ws-harness', provider: 'gemini', userPrompt: 'Read stale_optional_marker', capture: capture('gemini', 'optional-root', []) });
  assert.equal(initial.ok, true);
  assert.match(initial.provider_text, /stale_optional_marker/);

  fs.renameSync(linked, unavailable);
  const current = await prepareManagedSend(db, { workspaceId: 'ws-harness', provider: 'gemini', userPrompt: 'Read stale_optional_marker and current.md', capture: capture('gemini', 'optional-root', []) });
...
test('current chat synchronization failure blocks even when disk resources are current', t => {
  ...
  const result = await prepareManagedSend(db, { workspaceId: 'ws-harness', provider: 'gemini', userPrompt: 'Continue', capture: badCapture });
...
test('secret files and local-only roots are excluded from provider context while ordinary evidence still sends', t => {
  ...
  const result = await prepareManagedSend(db, { workspaceId: 'ws-harness', provider: 'chatgpt', userPrompt: 'What architecture evidence is available?', capture: capture('chatgpt', 'security', []) });
...
test('resource provenance hash matches the current bytes supplied to prepare-send', t => {
  ...
  const result = await prepareManagedSend(db, { workspaceId: 'ws-harness', provider: 'chatgpt', userPrompt: 'Read exact-name.txt', capture: capture('chatgpt', 'exact-file', []) });
...
test('an unrelated failed heavy-document representation does not block a verified current text turn', t => {
  ...
  const result = await prepareManagedSend(db, { workspaceId: 'ws-harness', provider: 'chatgpt', userPrompt: 'What does current.txt say?', capture: capture('chatgpt', 'optional-heavy-document', []) });
  assert.equal(result.ok, true);
...
test('partial historical coverage remains honest while a fresh snapshot can still be CURRENT', t => {
  ...
  const result = await prepareManagedSend(db, { workspaceId: 'ws-harness', provider: 'chatgpt', userPrompt: 'Use the fresh source', capture: capture('chatgpt', 'new-current', []) });
  assert.equal(result.ok, true);
...
test('visual queries select only the current image version by opaque resource/version ID', t => {
  ...
  const result = await prepareManagedSend(db, { workspaceId: 'ws-harness', provider: 'chatgpt', userPrompt: 'Inspect the visual layout in diagram.png', capture: capture('chatgpt', 'image-query', []) });
  assert.equal(result.ok, true);
...
test('delivery acknowledgement requires exact prepared identity and strong or corroborated provider acceptance', t => {
  ...
  const prepared = await prepareManagedSend(db, { workspaceId: 'ws-harness', provider: 'chatgpt', userPrompt: prompt,
...
test('a disk change after prepare prevents delivery acknowledgement and remains auditable', t => {
  ...
  const prepared = await prepareManagedSend(db, { workspaceId: 'ws-harness', provider: 'gemini', userPrompt: prompt,
...
test('attachment fallback reprepares the latest exact version and records fallback provenance', t => {
  ...
  const first = await prepareManagedSend(db, { workspaceId: 'ws-harness', provider: 'gemini', userPrompt: prompt, capture: capture('gemini', 'fallback-chat', []) });
  ...
  const second = await prepareManagedSend(db, { workspaceId: 'ws-harness', provider: 'gemini', userPrompt: prompt, capture: capture('gemini', 'fallback-chat', []),
...
test('an explicitly requested native attachment above the companion transfer bound fails closed with an honest reason', t => {
  ...
  const result = await prepareManagedSend(db, { workspaceId: 'ws-harness', provider: 'chatgpt', userPrompt: 'Attach oversized-design.docx', capture: capture('chatgpt', 'oversized-attachment', []) });
  assert.equal(result.ok, false);
